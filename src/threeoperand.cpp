...
as::ThreeOperand::ThreeOperand(as::Level *const lvlA, const std::string& cmdLineA, const uint32_t lineNumberA, as::ParseObjBase *const firstA, as::ParseObjBase *const secondA, as::ParseObjBase *const thridA, const uint32_t machienIdA) :
    as::ParseObjBase{lvlA, as::COMMANDCLASS::THREEOPERANT, cmdLineA, lineNumberA}, m_first{firstA}, m_second{secondA}, m_third{thridA}, m_machineCodeId{machienIdA}
{return;}

as::ThreeOperand::ThreeOperand(const as::ThreeOperand& srcA) :
    as::ParseObjBase{srcA.getLevel(), srcA.getCommandClass(), srcA.getReadCmdLine(), srcA.getFileLineNumber()}, m_first{srcA.m_first}, m_second{srcA.m_second},m_third{srcA.m_third}, m_machineCodeId{srcA.m_machineCodeId}
{return;}
...
as::ThreeOperand::operands_type_t as::ThreeOperand::setOperands(as::ParseObjBase *const firstA, as::ParseObjBase *const secondA, as::ParseObjBase *const thirdA)
{
...
as::ThreeOperand::operands_type_t as::ThreeOperand::setOperands(const as::ThreeOperand::operands_type_t& operandsA)
{
    ...
    as::ThreeOperand::operands_type_t t_ops = std::make_tuple(
        this->m_first, this->m_second, this->m_third);
...
std::string as::ThreeOperand::assemble(const boost::property_tree::ptree& ptreeA)
{
     ...
     */
    auto t_opt = boost::tokenizer<boost::escaped_list_separator<char>>(ptreeA.get<std::string>("Assembler_Property.ThreeOperator"));
            ...
            if(ptreeA.get<uint32_t>(opt) == m_machineCodeId)
            {
                switch(t_countVal)
                {
                    ...
                    case 1:
                        ...
                        break;
                    default:
                        throw as::AssemblerException("Unknown three-operation-command. Cannot select number of cache lines.", 8812);
                        break;
                }
                break;
            }
    ...
    if(m_first->getCommandClass() == as::COMMANDCLASS::CONSTANT)
        ...
        Expected constant integer value or integer variable for first operand", 8813);
    ...
    if(m_second->getCommandClass() == as::COMMANDCLASS::CONSTANT)
        ...
        throw as::AssemblerException("Wrong parameter format for three-operation-command.\n \
        Expected constant integer value or integer variable for second operator", 8814);
    ...
    if(m_second->getCommandClass() == as::COMMANDCLASS::CONSTANT)
        ...
        throw as::AssemblerException("Wrong parameter format for three-operation-command.\n \
        Expected constant integer value or integer variable for third operator", 8814);
