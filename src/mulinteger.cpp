...
MulInteger::MulInteger(Level *lvlA, const std::string &cmdLineA, const uint32_t &lineNumberA, ParseObjBase *firstA,
...
MulInteger &MulInteger::operator=(const MulInteger &rhsA)
{
...
MulInteger &MulInteger::operator=(MulInteger &&rhsA)
{
...
bool MulInteger::processOperation()
{
    ...
    if (t_first == nullptr)
        throw AssemblerException("First argument of MULI is not a variable.", 8504);

    if (t_second == nullptr)
        throw AssemblerException("Second argument of MULI is not a constant.", 8505);
